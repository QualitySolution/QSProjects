		...
		where TNode : JournalEntityNodeBase
	{
		...
		private string GetEntityTitleName()
		{
			...
			if(names == null || string.IsNullOrWhiteSpace(names.Nominative)) {
				throw new ApplicationException($"Для типа {typeof(TEntity)} не проставлен атрибут AppellativeAttribute, или в атрибуте не проставлено имя Nominative, из-за чего невозможно разрешить правильное имя документа для отображения в журнале с конфигурацией документов по умолчанию.");
			}
		...
		/// <summary>
		/// Добавление конфигурации документа с не стандартным определением диалогов, с определенным идентификатором и именем взятым из описания сущности
		/// </summary>
		/// <returns>Конфигурация документа</returns>
		/// <param name="docIdentificationFunc">Уникальный идентификатор типа документа, должен возвращать true только для тех строк для которых должен открываться выбранный тип диалога и больше никакой другой</param>
		...
		/// <summary>
		/// Завершение конфигурации документа, проверка конфликтов и запись конфигурации в модель
		/// </summary>
