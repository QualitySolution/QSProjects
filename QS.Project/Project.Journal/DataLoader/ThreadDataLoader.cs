...
namespace QS.Project.Journal.DataLoader
{
		...
		where TNode: class
	{
		...
		protected int? GetPageSize => DynamicLoadingEnabled ? (ItemsCountForNextLoad ?? PageSize) : (int?)null;
		...
		private void ReadLoadersInSortOrder()
		{
			...
			while (loaders.Any(l => l.NextUnreadedNode() != null)) {
				...
				if(ItemsCountForNextLoad.HasValue && ItemsCountForNextLoad <= readedNodes.Count) {
					break;
				}
