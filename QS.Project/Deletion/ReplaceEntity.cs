			...
			where TEntity : IDomainObject
		{
			...
			foreach(var depend in delConfig.RemoveFromItems) {
				...
				foreach(var item in GetLinkedEntities(uow, depend, fromE)) {
					...
					//Это правило используется для коллекций связей многие к многим. Объект на который заменяем уже может быть добавлен в коллекцию, добавлять его повторно не имеет смысла.
