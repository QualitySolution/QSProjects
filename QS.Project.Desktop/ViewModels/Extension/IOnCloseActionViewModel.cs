...
namespace QS.ViewModels.Extension
{
	...
	/// ViewModel получить вызов перед получением события о закрытии страницы внешними подписчиками, то есть ViewModel
	/// имеет возможность перед закрытием подготовить данные. По этой причине в методе нельзя производить деструктивные действия
