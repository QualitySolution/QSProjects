node {
   ...
   stage('My-FyiReporting') {
      checkout changelog: true, poll: true, scm: [$class: 'GitSCM', branches: [[name: '*/release/1.6']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'My-FyiReporting']], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/QualitySolution/My-FyiReporting.git']]]
   ...
   stage('Build Net4.x') {
    ...
    stage('Test net4.x'){
       try {
            ...
            sh "xvfb-run mono ${PACKAGES_LOCATION}/nunit.consolerunner/3.15.0/tools/nunit3-console.exe QSProjects/QS.LibsTest/bin/Debug/QS.LibsTest.dll"
       } catch (e) {}
       finally{
